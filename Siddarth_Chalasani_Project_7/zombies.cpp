...
bool Zombie::getAttacked(int dir)  // return true if dies
{
    ...
    wasAttacked = true;
    return !m_arena->determineNewPosition(m_row, m_col, dir);
}
...
void Player::moveOrAttack(int dir)
{
    ...
    {
        case UP:
            if (m_arena->numZombiesAt(row()-1, col()))
        ...
        case DOWN:
            ...
            else if (row() < m_arena->rows())
                m_row++;
            break;
        case LEFT:
            if (m_arena->numZombiesAt(row(), col()-1))
    ...
    for (r = 0; r < rows(); r++)
        for (c = 0; c < cols(); c++) {
            if (numZombiesAt(r + 1, c + 1) == 1)
                grid[r][c] = 'Z';
            else if (numZombiesAt(r + 1, c + 1) > 1) {
